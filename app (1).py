...
# ---------------------------------------------------------------------------
# الاتصالات — بشكل آمن: أي فشل هنا يظهر كرسالة واضحة بدل ما يوقع الصفحة كلها
# ---------------------------------------------------------------------------
REQUIRED_SECRETS = ["HF_TOKEN", "qdranturl", "qdrantapi", "OPENROUTER_API_KEY"]
missing_secrets = [k for k in REQUIRED_SECRETS if k not in st.secrets]

if missing_secrets:
    st.error(
        "⚠️ الـ Secrets الناقصة دي: **" + ", ".join(missing_secrets) + "**\n\n"
        "روح App settings → Secrets وضيفها بنفس الأسماء دي بالظبط (حساسة لحالة الأحرف)."
    )
    st.stop()
...
try:
    ...
    st.error(f"⚠️ فشل الاتصال بأحد الخدمات وقت بدء التشغيل:\n\n`{e}`")
    st.stop()
