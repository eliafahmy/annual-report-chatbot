...
if question:
    ...
    with st.chat_message("assistant", avatar="🤖"):
        ...
        if not results:
            ...
            context = "\n".join(r["content"] for r in results)
            system_prompt = "You are a precise financial-report analysis assistant. Answer strictly using the provided context — never invent numbers or facts not present in it. If the user asks in Arabic, reply in Arabic; otherwise match the question's language."
            ...
            if any(k in q_lower for k in ["revenue", "trend", "growth", "over the years", "إيراد", "عبر السنين", "بالسنوات"]):
                ...
                if trend_data:
                    ...
                    fig.update_layout(
                        title="Total Revenue Trend Over Years (من التقرير الفعلي)",
